/*
	...
	풀이 : 세그먼트 트리를 두개만들어 최솟값과 최댓값을 구한다.
*/
#include<iostream>
#include<vector>
#include<algorithm>
...
int MAX(int L, int R, int node, int nl, int nr)
{
	if (R<nl || L>nr) return -1;
	if (L <= nl && nr <= R) return arr2[node];
	int mid = (nl + nr) / 2;
	return max(MAX(L, R, node * 2, nl, mid), MAX(L, R, node * 2 + 1, mid + 1, nr));
}
...
int main()
{
	...
	while(m--)
	{
		int a, b;
		cin >> a >> b;
		cout << MIN(a-1, b-1, 1, 0, k-1) << " " << MAX(a-1,b-1,1,0,k-1) << endl;
	}
